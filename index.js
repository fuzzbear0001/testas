...
fs.readdir("./src/commands", (err, files) => {
    ...
    console.log(`[bhbotlist.xyz]: ${files.length} command loaded.`);
    files.forEach(f => {
...
client.on('ready',async () => {
    console.log("[bhbotlist.xyz]: Bot successfully connected as "+client.user.tag+".");
    let botsSchema = require("./src/database/models/botlist/bots.js");
    const bots = await botsSchema.find();
    client.user.setPresence({ activity: { type: 'WATCHING', name: 'Calypso Bot List | '+bots.length+' bots' }, status: "online" });
});
/*=======================================================================================*/
