...
console.log("[bhbotlist.xyz]: Admin/Botlist/Confirm Bot router loaded.");
...
app.get("/admin/confirm/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.params.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>'s bot named <@${botdata.botID}> has been approved by <@${req.user.id}>`)
        client.users.cache.get(botdata.ownerID).send(`Your bot named **${bota.tag}** has been approved by <@${req.user.id}>`)
    });
